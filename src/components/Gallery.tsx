...
// Single tile with skeleton until image loads
const ImageTile = ({ src, idx, onClick }: { src: string; idx: number; onClick: () => void }) => {
  const [loaded, setLoaded] = useState(false);
  return (
    <button
      onClick={onClick}
      className="rounded-xl overflow-hidden bg-white border border-gray-200 hover-lift-md hover-glow animate-fadeInUp"
      style={{ display: 'inline-block', animationDelay: `${idx * 40}ms` }}
    >
      <div className="relative h-[140px] md:h-[150px] w-[220px] md:w-[260px]">
        {/* Skeleton overlay */}
        {!loaded && (
          <div className="absolute inset-0 bg-gray-100 loading" />
        )}
        {/* Keep image in DOM so it loads; fade it in when ready */}
        <img
          src={src}
          alt="gallery"
          className={`h-[140px] md:h-[150px] w-[220px] md:w-[260px] object-cover block transition-opacity duration-300 ${loaded ? 'opacity-100' : 'opacity-0'}`}
          loading="lazy"
...
const Section = ({ title, items, onImageClick }: { title: string; items: string[]; onImageClick: (src: string, idx: number) => void }) => (
  <div className="space-y-2">
    <h2 className="text-base font-semibold text-gray-900 animate-fadeInUp anim-delay-0">{title}</h2>
    <div className="flex flex-wrap gap-2.5">
      {items.map((src, i) => (
        <ImageTile key={i} src={src} idx={i} onClick={() => onImageClick(src, i)} />
      ))}
    </div>
  </div>
);
...
const Gallery: React.FC = () => {
  ...
  return (
    <div className="h-[calc(100vh-4rem)] overflow-hidden px-1 lg:px-2 py-3 md:py-3">
      {/* Header */}
      <div className="flex items-center justify-between mb-4 animate-fadeInUp anim-delay-0">
        <div>
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            />
            <button className="absolute right-1 top-1/2 -translate-y-1/2 px-2 py-1 text-sm rounded-md bg-gray-900 text-white hover:bg-gray-800 hover-lift-md">
              Find
            </button>
          </div>
        </div>
