    ...
    <div className={embedded ? 'text-[14px]' : 'min-h-screen bg-gray-50 dark:bg-gray-900 dashboard-rounded overflow-y-auto'}>
      ...
      <div className={embedded ? '' : 'flex pt-16 overflow-auto'}>
            ...
            {sidebarItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`relative group dashboard-pill flex ${sidebarCollapsed ? 'justify-center' : 'items-center'} w-full py-2.5 px-3 rounded-lg transition text-sm font-medium ${
                    isActive
                ...
                >
                  <span className={`${isActive ? 'text-gray-900 dark:text-white' : 'text-gray-400'} group-hover:text-gray-900 dark:group-hover:text-white`}>
                    {item.icon}
                  </span>
                  {!sidebarCollapsed && <span className="ml-3">{item.label}</span>}
                  {sidebarCollapsed && (
                    <span className="pointer-events-none whitespace-nowrap absolute left-full top-1/2 -translate-y-1/2 ml-2 px-2 py-1 rounded-md text-xs bg-gray-900 text-white dark:bg-white dark:text-black shadow-lg opacity-0 group-hover:opacity-100 transition-opacity">
                      {item.label}
                    </span>
                  )}
                </button>
                    ...
                    return (
                      <button
                        ...
                        className={`relative dashboard-pill flex items-center w-full py-2.5 px-3 transition text-sm font-medium ${
                          isActive ? 'text-gray-900 bg-transparent ring-1 ring-gray-900/10' : 'text-gray-700 hover:bg-white/5 hover:ring-1 hover:ring-gray-900/10'
                        }`}
                      >
        ...
        <div className={embedded ? 'text-[14px]' : 'flex-1 p-4 lg:p-5 text-[14px] min-h-[calc(100vh-4rem)] overflow-visible'}>
          ...
          {/* Header */}
          <div className="mb-4">
            <h1 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-0.5 animate-fadeInUp">API Connections</h1>
            <p className="text-gray-600 dark:text-gray-300 text-xs animate-fadeInUp anim-delay-1">Easily connect your cloud storage providers and manage them in one place.</p>
          </div>
          ...
          <div className="mb-4">
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100 animate-fadeInUp">Top Providers</h2>
